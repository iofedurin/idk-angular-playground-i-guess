...
### Decision tree: нужен ли effect()?

```
Нужна реакция на изменение сигнала?
  │
  ├─ НЕТ → ngOnInit() / event handler / submission action
  │
  └─ ДА → Результат — новое значение (без side-effect)?
           ├─ ДА → computed()
           │
           └─ НЕТ → Императивный DOM API (showModal, focus, scrollTo)?
                     ├─ ДА → effect() ✓
                     │
                     └─ НЕТ → Реактивная загрузка данных
                               (computed params → store.loadPage)?
                               ├─ ДА → effect() ✓
                               │
                               └─ НЕТ → Скорее всего не нужен effect.
                                         Пересмотреть: навигация? → event handler.
                                         Cross-store sync? → guard + registry.
```
